...
**Domain:** `HR / Recruiting` · **Suite:** `HR & Recruiting Suite` · **Accent:** `#fbbf24`
...
**Candidate Scorer** is a standalone, browser-based AI workflow app for **HR / Recruiting**. It turns structured inputs, uploaded files, and project-specific settings into reviewable outputs using a deterministic local engine plus optional local/cloud LLM enhancement.
...
|---|---:|:---:|---|---|
| `cv`<br>CV | text | Yes | — | Affects input: CV. |
| `job_description`<br>job description | text | Yes | — | Affects input: job description. |
...
| `must_have_skills`<br>must-have skills | textarea | No | — | Affects customization: must-have skills. |
| `score_weights`<br>score weights | text | No | — | Affects customization: score weights. |
| `country_hiring_rules`<br>country hiring rules | select | No | United States | Affects customization: country hiring rules. |
| `anonymization`<br>anonymization | text | No | — | Affects customization: anonymization. |
...
- **country hiring rules**: United States, United Kingdom, Egypt, UAE, Saudi Arabia, Germany, Custom
...
uvicorn app.main:app --host 127.0.0.1 --port 9104
```
...
| `AI_SUITE_PORT` | Port for this project GUI, default `9104`. |
...
```bash
curl http://127.0.0.1:9104/api/health
```
...
```bash
curl -X POST http://127.0.0.1:9104/api/run \
...
```bash
AI_SUITE_HOST=0.0.0.0 AI_SUITE_PORT=9104 uvicorn app.main:app --host 0.0.0.0 --port 9104
```
...
- `plugins/candidate-scorer.json` for plugin metadata.
